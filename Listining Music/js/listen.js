...
function handleProgress() {
	...
	playHead.style.marginLeft =
		(percent / 100) * (timeLine.offsetWidth - playHead.offsetWidth) + 'px'
	playedBar.style.width =
		(percent / 100) * (timeLine.offsetWidth - playHead.offsetWidth) + 'px'
	let currentTimeMin = ('0' + parseInt(music.currentTime / 60)).slice(-2)
	let currentTimeSec = (
		'0' + parseInt(music.currentTime - currentTimeMin * 60)
	).slice(-2)
	let durationMin = ('0' + parseInt(music.duration / 60)).slice(-2)
	let durationSec = ('0' + parseInt(music.duration - durationMin * 60)).slice(
		-2
	)
	playTime.innerHTML =
		currentTimeMin +
...
function displayTrackName() {
	fetchUrl(order[0].name).then(res => {
		trackName.textContent = res.data
		console.log(res)
	})
	displayAnimation()
}
...
function displayTimeUpdate(status) {
	trackName.innerHTML = `${
		status
			...
			: "<i class='glyphicon glyphicon-backward' style='padding-right: 5px'></i>"
	} ${Math.floor(music.currentTime / 60) < 10 ? '0' : ''}${Math.floor(
...
function handleBuffer() {
	if (music.currentTime > 0) {
		bufferedBar.style.flexBasis =
			Math.round((music.buffered.end(0) / music.duration) * 100) + '%'
	}
}

var order = shuffle(soundtracks)
...
function isMusicLoading() {
	return (
		music.readyState !== music.HAVE_FUTURE_DATA &&
...
const musicStateEvents = ['play', 'pause', 'playing', 'seeking', 'seeked']
